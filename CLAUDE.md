...
**当前里程碑：M2 · 页面真实操作（可写纵切链）**（6 仓同名 Milestone 承接）。
**目标（达成判据）**：在 M1「只读骨架贯通」之上补**第一批"可写"纵切链**——让前端页面能**真实操作业务功能**（填表单 → 真后端 → 落库 → 回显，且跨租户隔离）。三条链：①数据集成·数据源接入（连 MySQL，锚点）②数据目录·资产登记 ③组织管理·成员；每条都「浏览器 → gateway(OIDC+租户头) → 真实后端 → PG → 页面回显」。**接受功能不完整，但架构不可有错误、基础骨架不可返工**。
总纲与各仓交付清单见 **[`docs/milestones/M2-页面真实操作-可写纵切链.md`](docs/milestones/M2-页面真实操作-可写纵切链.md)**；跨仓集成由主仓 tracking issue #50 统筹。
> M1（单命名空间端到端贯通）已于 2026-06-24 收官，详见 **[`docs/milestones/M1-单命名空间端到端贯通.md`](docs/milestones/M1-单命名空间端到端贯通.md)**。
...
**不可返工的关键决策（D1–D9）**：D1 单 Keycloak User + 多 Org Membership；D2 切换租户=重新换 token、`X-Tenant-Id` 唯一；D3 白标部署级注入、不按租户运行期换肤；D4 默认品牌 emerald；D5 子仓交付 image、主仓 owns charts；D6 身份后端 M1 只做真实 OIDC 登录 + control-plane 真实 IdentityProvisioner（余 stub/后置）。**M2 新增**：D7 数据源凭据应用层 AES-GCM 加密存 PG、主密钥部署期注入不入库、预留 `secret_ref` 间接层；D8 连接器中立（MySQL 是第一个 SPI 方言实现、不写死）；D9 写链按 `X-Tenant-Id` 强制隔离、组织管理=`apps/console` 角色门控的租户自管理（成员 SoT 在 Keycloak）。D1–D6 见 M1 总纲 §2，D7–D9 见 M2 总纲 §2 与 `docs/architecture/05`。
